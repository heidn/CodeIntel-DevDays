...
0. **LSP `workspace/symbol` still only returns ambient-lib symbols, not project symbols — trace-on-TS is still broken.** After all three LSP fixes above, `workspace/symbol('Em')` returns 42 hits but `workspace/symbol('ShopAddTab')` returns 0 — i.e. tsserver answers from `lib.*.d.ts` ambient definitions but the 168 `didOpen`'d project files never make it into the navto index. A diagnostic `LogInformation` is currently live in [LspSession.cs](src/CodeIntel.Server/Services/Language/Lsp/LspSession.cs) `ResolveEntryPointAsync` (`"LSP workspace/symbol('{Query}') → {Count} raw symbols"`) — **downgrade to `LogDebug` or remove once fixed.** Next step: bump the `…LanguageBackends.Lsp` log category to `Debug` and read tsserver's stderr (already forwarded via `LogDebug` in `LspSession.StartAsync`) — likely a `didOpen` URI format issue (`PathToUri` produces `file:///C:/…`; verify tsserver accepts it, esp. paths with `(parens)` like Expo Router's `app/(app)/`) or tsserver needs the project opened explicitly. Until this is fixed, trace mode + class/method context-requests on TS workspaces fall back to text search.
1. **`<done/>` still not emitted despite the strengthened prompt — fix the cache gate in code, not the prompt.** The 186s chunked find-business-rules run produced clean per-chunk rationales but `reachedDone=False` → cache skipped. The 7B model treats the rationale sentence as end-of-turn. Prompt strengthening alone won't fix this. Real fix: in [InvestigationOrchestrator.cs](src/CodeIntel.Server/Services/InvestigationOrchestrator.cs) `runLooksHealthy`, **drop the `lastIterationReachedDone` clause** — `IncompleteFindingCount`/`MalformedFindings` are the *direct* truncation signals; `<done/>` is a courtesy marker the model emits unreliably. A stream that ended naturally with zero incomplete/malformed findings is healthy and cacheable even without the marker. Keep `reachedDone` on the `completed` event for the UI but don't let it block caching.
...
### Remaining test pass still deferred

4. **Trace mode end-to-end on a TS file** — LSP init bug is fixed but not yet re-verified end-to-end. Confirm `prepareCallHierarchy` + `callHierarchy/incomingCalls`/`outgoingCalls` populate.
5. **SQL preset end-to-end on `test-data/sql`.** Pick `orders_api.pkb` or `payroll_pkg.pkb`, run each of the 4 SQL-tuned presets, verify the PL/SQL dep-resolver attaches table DDL.
