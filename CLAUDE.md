...
**Reliability + LSP repair pass shipping (2026-05-14 session).** Three next-session priorities closed — TS Metrics now returns `Supported=false` with an explicit UI placeholder instead of a misleading `0/0` ([MetricsService](src/CodeIntel.Server/Services/MetricsService.cs) + [MetricsPanel](src/CodeIntel.Server/ClientApp/src/components/MetricsPanel.tsx)); the find-bugs "When you find nothing" rationale block is hoisted above the don't-emit rules; all 5 find-style prompts now make `<done/>` explicitly mandatory after a rationale sentence. End-to-end testing then surfaced and fixed three more: (1) the **idle watchdog counted cold-start prompt-eval as idle** — a single shared `idleCts` armed at 90s killed chunk 2 the instant chunk 1's >90s prompt-eval finished; fixed with `FirstTokenTimeoutSeconds` (240s) for the pre-stream arm vs the tight `IdleTokenTimeoutSeconds` for inter-token gaps, overall ceiling now scales `× chunkCount`; (2) **LSP sent JSON-RPC params positionally** — every `LspSession` call wrapped params in `new object[]{}`, so `typescript-language-server` rejected `initialize` and **all** TS semantic features silently fell back to text search; all 11 sites now use `InvokeWithParameterObjectAsync`; (3) **LSP opened no files before querying** so tsserver never loaded a project — added `WarmUpProjectAsync` (opens source files on init, polls `workspace/symbol` until live). Verified: the original chunked find-business-rules repro now runs 186s clean (was idle-cancelling); SQL pipeline end-to-end (ANTLR parse → dep-resolver attaches table DDL → find-bugs-sql streams a well-formed finding + `<done/>`). A follow-up end-to-end pass (2026-05-14, tests 4 + 5) then verified the SQL pipeline across all 4 presets and **re-scoped the TS-trace issue**: location-based trace (`prepareCallHierarchy` + `callHierarchy/*`) works end-to-end on a 172-file Expo app — only *name-based* entry resolution is still broken (`workspace/symbol` is blind to project symbols). **Still open:** name-based TS entry resolution, the `reachedDone` cache-gate (re-confirmed denying a clean run), chunked companion-file drop, find-business-rules over-rationalizing, and low 7B finding-recall on SQL presets. See "Pickup Priorities".
...
5. **First inference is slow — and the idle watchdog must not mistake it for a hang.** CPU/Vulkan model warmup takes 30-60s, and cold-start *prompt-eval* of a budget-sized context (~5K tokens) streams nothing for ~100s+ before the first token. The idle watchdog therefore arms each LLM call with `Analysis:FirstTokenTimeoutSeconds` (default 240) and only switches to the tight `Analysis:IdleTokenTimeoutSeconds` (default 90) once tokens start flowing. A single shared `idleCts` covers the whole run including every chunk — if a run idle-cancels immediately after a chunk completes, the watchdog fired *during* that chunk's prompt-eval; raise `FirstTokenTimeoutSeconds`. The overall ceiling scales: `OverallTimeoutSeconds × chunkCount` for chunked runs.
...
14. **LSP JSON-RPC params are by-name, not positional.** Every `initialize` / `workspace/symbol` / `textDocument/*` / `callHierarchy/*` call must go through `JsonRpc.InvokeWithParameterObjectAsync` (or `NotifyWithParameterObjectAsync`), which sends `params: obj`. The plain `InvokeWithCancellationAsync(method, object[], ct)` sends `params: [obj]` and `typescript-language-server` rejects it with *"defines parameters by name but received parameters by position"* — which fails `initialize` and disables **all** TS semantic features (trace, definition, callers/callees) with only a `WRN` in the log. `shutdown`/`exit` take no params and can stay on the positional overload. If TS trace/definition silently falls back to text search, check [LspSession.cs](src/CodeIntel.Server/Services/Language/Lsp/LspSession.cs) for a stray positional call first.
...
- Load any folder of PL/SQL files as a `Language.Sql` workspace (existing file-scan path; no new loader needed). Recognized extensions live in one place — `PlSqlFileExtensions.All` in [WorkspaceModels.cs](src/CodeIntel.Server/Models/WorkspaceModels.cs): `.sql .pkg .pkb .pks .pls .tbl .trg .idx`. The workspace loader (`PlSqlBackend` → `FileScanner`), `LanguageDetector`, `FileChunker`, `ContextBuilder` dep-resolution, `PlSqlRepoResolver`, `MetricsService`, and the report/prompt code-fence hints all route through that constant — add a new Oracle extension there and every consumer picks it up.
...
### Top of the next session — issues uncovered by the 2026-05-14 end-to-end test pass

The 2026-05-13 follow-up items (TS metrics placeholder, find-bugs hoist, `<done/>` prompt strengthening) all **shipped** this session. A fresh end-to-end pass then ran and surfaced two **blocker bugs (now fixed)** plus three **open issues**:

**Fixed this session:**
- ✅ **Idle-watchdog counted prompt-eval as idle.** Chunked runs idle-cancelled the moment chunk 1 finished — the watchdog fired during chunk 1's >90s cold-start prompt-eval, then chunk 2's `ct.ThrowIfCancellationRequested()` threw. Fixed: `FirstTokenTimeoutSeconds` (default 240) arms the watchdog before each LLM call; per-token resets use the tight `IdleTokenTimeoutSeconds`. Overall ceiling scales `× chunkCount`. Re-tested: chunked find-business-rules ran 186s clean, no cancel.
- ✅ **LSP sent JSON-RPC params positionally.** Every `LspSession` call wrapped params in `new object[] { … }`, so `typescript-language-server` rejected `initialize` — disabling **all** TS semantic features with only a `WRN`. Fixed: all 11 call sites use `InvokeWithParameterObjectAsync` / `NotifyWithParameterObjectAsync`. See gotcha #14.
- ✅ **LSP `workspace/symbol` returned nothing — no project loaded.** `initialize` succeeded but symbol queries came back empty: tsserver only loads a project once a file is opened, and `LspSession` opened none before querying. Added `WarmUpProjectAsync` — opens the workspace's source files on init then polls `workspace/symbol` until the index answers. (First cut opened a single root-level `*.config.ts`, which loads only an *inferred* project blind to `src/`; now opens the full source set.)

**Open issues (in priority order):**
0. **LSP name-based entry resolution is broken (`workspace/symbol` blind to project symbols) — but location-based trace-on-TS WORKS.** Re-scoped by the 2026-05-14 test-4 pass on `pantryprep.client` (172-file Expo app). What works end-to-end: **location-based** trace entry points (`{filePath, line, character}` → `prepareCallHierarchy` → `callHierarchy/incomingCalls|outgoingCalls`). Verified both directions — callers trace resolved `extractErrorMessages` + its `member-edit.handleSave` caller; callees trace from `handleSave` produced 12 nodes / 14 edges with correct back-edge detection, *including through Expo Router `app/(app)/(admin)/` parens paths*. So the "Trace from here" UI path (which passes a location) is fine. What's still broken: **name-based** resolution — `POST /api/trace/candidates` returns `[]` for every project symbol and a name-only `/run` fails with `Could not resolve entry point: <name> / null:null`. Root cause unchanged: `workspace/symbol` answers only from `lib.*.d.ts` ambient definitions, not the 168 `didOpen`'d project files. Note: the diagnostic `LogInformation` ("…raw symbols") in [LspSession.cs](src/CodeIntel.Server/Services/Language/Lsp/LspSession.cs) `ResolveEntryPointAsync` Path A **never fired** during the test pass — name resolution may be bailing *before* the `workspace/symbol` query (check the candidates code path / `TraceWalker.ResolveCandidatesAsync`). Next step: still the `didOpen` URI / explicit-project-open investigation, but priority is lower now that location-based works — the UI's typed-method-name input is the only thing blocked. (Reminder: API `TraceEntryPoint.Line` is **1-based** — `LspSession` does `line - 1` to reach LSP's 0-based convention.)
1. **`<done/>` still not emitted despite the strengthened prompt — fix the cache gate in code, not the prompt.** *Re-confirmed by the 2026-05-14 test-5 pass:* `find-bugs-sql` on `payroll_pkg.pkb` ran fully clean (1 finding, `incomplete=0 malformed=0`) yet still logged `Skipping result cache write … reachedDone=False` — a healthy run denied caching purely because the 7B model didn't emit the marker. (Runs that *did* emit `<done />` — `efficiency-review`, `find-business-rules-sql` — cached fine, so the gate is purely marker-dependent.) Real fix: in [InvestigationOrchestrator.cs](src/CodeIntel.Server/Services/InvestigationOrchestrator.cs) `runLooksHealthy`, **drop the `lastIterationReachedDone` clause** — `IncompleteFindingCount`/`MalformedFindings` are the *direct* truncation signals; `<done/>` is a courtesy marker the model emits unreliably. A stream that ended naturally with zero incomplete/malformed findings is healthy and cacheable even without the marker. Keep `reachedDone` on the `completed` event for the UI but don't let it block caching.
2. **Chunked runs silently drop companion files.** The repro selected 4 files; `analyzedFiles` came back as just `[ShopAddTab, ShopListTab]` — once ShopAddTab chunked, its chunk + ShopListTab filled the 5000-token budget and ShopModeView/types.ts were dropped (`Chunked build: skipping companion … no budget remaining`). Only signalled in the server log. Fix: surface dropped-companion files in a `status` event + the `completed` payload so the UI can warn "2 of 4 files fit the budget."
3. **find-business-rules over-uses the rationale-on-empty hatch.** 0 findings on a 672-line shopping-list component that almost certainly has quantity/category/dedup rules. Same failure class as the old find-bugs over-restriction: the escape hatch makes "say nothing" the path of least resistance. Needs prompt iteration — possibly per-chunk the model can't see enough to find cross-cutting rules, so consider whether chunked files should get a "rules may span chunks — report what this slice shows" nudge.

### Remaining test pass — ✅ done 2026-05-14

4. ✅ **Trace mode end-to-end on a TS file — location-based path verified, name-based path still broken.** Ran on `C:\Users\heidn\repos\PantryPrep\pantryprep.client` (172-file Expo app). `prepareCallHierarchy` + `callHierarchy/incomingCalls`/`outgoingCalls` all populate correctly for **location-based** entry points; callees trace from `handleSave` gave 12 nodes / 14 edges / correct dashed back-edges / accurate per-node synopses / `truncated` flag, even through `app/(app)/(admin)/` parens paths. **Name-based** resolution still fails (see open issue #0). Net: trace-on-TS is *not* fully blocked — only the typed-method-name input is.
5. ✅ **SQL preset end-to-end on `test-data/sql` — all 4 presets ran clean, dep-resolver confirmed.** Loaded the 7-file SQL workspace; ran `find-bugs-sql`/`payroll_pkg.pkb`, `efficiency-review`/`order_reports_pkg.pkb`, `cleanup-stored-proc`/`hr_legacy_pkg.pkb`, `find-business-rules-sql`/`employees_tables.sql`. ANTLR parser fired on every run (`Parsed PL/SQL refs … N tables`); the dep-resolver attached the right DDL each time (`PL/SQL deps: 1 appended` — payroll→`employees_tables.sql`, order_reports→`tables.sql`, hr_legacy→`employees_tables.sql`); skill router fired `plsql-cursor`; save-to-repo wrote a well-formed MD with the correct preset-aware Copilot Next Step. **Caveats** (all known 7B-model weakness, not pipeline bugs): finding *recall* is low — `find-bugs-sql` caught 1 of ~6 planted bugs; `cleanup-stored-proc` tunnel-visioned on "magic number" and emitted 12 near-dupes of one class, hitting `MaxResponseTokens` mid-finding (degraded run, correctly *not* cached); `efficiency-review` 5 findings (~3 solid); `find-business-rules-sql` best at 7 of ~11 expected rules. Also re-confirmed open issue #1 (clean `find-bugs-sql` run denied caching on `reachedDone=False`).

### Pre-tonight backlog

6. **Auth (C1) + configurable CORS (B5).** With persistence + rate limiting + ignored-findings + Save-to-repo all shipping, the remaining gating concern before OpenShift is identity. Plan: Windows Auth via IIS passthrough on the internal network, plus a small middleware shim that scopes `WorkspaceController.Browse` (currently still leaks the host filesystem) to an allowlist of root prefixes. Move CORS origins to `Cors:AllowedOrigins` while in `Program.cs`.
7. **Smoke-test the Metrics tab end-to-end on a C# / PL/SQL workspace** (TS metrics now show a "not supported" placeholder — see this session's fix). Load `CodeIntel.sln`, verify cyclomatic complexity numbers look sane on known-complex methods. Then load a PL/SQL folder and verify cursor + WHEN OTHERS counts. Cache invalidation: edit a file, re-compute, confirm numbers update.
8. **Test coverage for the hardening surface.** SQLite-backed stores, `ResultCache`, `IgnoredFindingsStore`, `PathSafety`, `SecretScrubber`, `FindingsAggregator`, `FindingsComparer`, `SkillRouter`, `MetricsService` — none of these have unit tests yet. Existing xunit project covers PL/SQL parser/resolver/builder + `FileChunker` + `FindingStreamParser`. Target: 70% line coverage on `Services/` before the OpenShift cutover (see [docs/REVIEW-ENHANCEMENTS.md](docs/REVIEW-ENHANCEMENTS.md) §E).
9. **Verify trace-mode in the UI on a richer C# project.** Local smoke-tests passed on `CodeIntel.sln` (Callees depth=1 → 8 nodes ~1m25s; Both depth=2 → 20 nodes ~3m). Want to confirm the experience on something bigger — controller/service entry points in a real LOB app. Now interesting: does the `MaxTotalNodes=100` cap fire? Does the overload-candidate picker get hit? Does the callers cache change wall-clock noticeably?
10. **PL/SQL repo-mode UI smoke-test + Oracle live (v2).** v1 (repo-only) is shipping with an ANTLR-backed parser — load a real PL/SQL repo, pick a stored proc, run each of the 4 SQL presets, verify the parser/resolver attaches the right object definitions and the Copilot Next Step briefs read cleanly. Then v2: add `Oracle.ManagedDataAccess.Core`, wire a `SqlOptions` (connection string) + `IOracleSchemaService` that fulfils the `OracleObject` context-request with live `ALL_TABLES` / `USER_SOURCE` data when the repo doesn't have the DDL committed. Pairs with the trace-mode DbAccess NodeKind (cross-workspace augmentation) once the LSP rewrite lands.
11. **Whole-repo dead-code detection.** Tree-sitter to enumerate functions, LSP for references, LLM only for ambiguous cases (reflection, DI, dynamic dispatch). Cheap once LSP is in place.
12. **Business documentation mode.** Walk top-level entry points → trace → catalog. Builds directly on the trace pipeline.
13. **File-backed skills.** `SkillRouter` already ships with content-keyed predicates; the next step is moving the hard-coded addenda into `Prompts/skills/*.md` and routing by the same predicates.
14. **Dockerfile + OpenShift** — multi-stage build (npm → dotnet publish), volume mounts for `/models` + `/data` (SQLite), ConfigMap for runtime tuning, `/readyz` wired to the readiness probe.
